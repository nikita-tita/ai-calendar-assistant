...
- **Модалка настроек** с возможностью расширения
- **Кнопка "Написать нам" 💬** — переход в чат поддержки t.me/iay_pm
- **Информационный popup** перед переходом в чат с описанием что можно отправить

### Technical
- Файл: `app/static/index.html`
- Версия WebApp: `2025-12-04-v3` (UI: v982)
- Использует `tg.showPopup()` для нативного Telegram popup
- Использует `tg.openTelegramLink()` для открытия чата в Telegram

### UX Flow
1. Пользователь нажимает ⚙️ в header
2. Открывается модалка "Настройки"
3. Нажимает "Написать нам"
4. Показывается popup с текстом о возможностях обратной связи
5. При нажатии "Перейти в чат" открывается t.me/iay_pm
