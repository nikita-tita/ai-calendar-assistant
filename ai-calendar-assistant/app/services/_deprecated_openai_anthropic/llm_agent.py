"""LLM Agent service using Anthropic Claude."""
...
class LLMAgent:
    ...
    Uses Anthropic Claude with function calling to extract structured
    event information from user queries.
    """

    def __init__(self):
        """Initialize LLM agent with Claude client."""
...
"""
    ...
    async def extract_event(
        ...
        """
        logger.info("llm_extract_start", user_text=user_text, user_id=user_id)
...
"""
            ...
            # Create dynamic system prompt with current date (without events - they go to user message)
...
"""
            ...
            # Define tool for Claude with dynamic event_id enum
            tools = [{
            ...
            event_dto = self._parse_claude_response(
                response,
                user_text,
                start_time,
                end_time,
            ...
            logger.error("llm_extract_error", error=str(e), exc_info=True)
    ...
    def _parse_claude_response(
        ...
        """Parse Claude API response into EventDTO."""

        # Find tool use in response
        tool_use = None
        for content in response.content:
            if content.type == "tool_use" and content.name == "set_calendar_action":
                tool_use = content
                break

        if not tool_use:
            # No tool use, return clarify
            return EventDTO(
                intent=IntentType.CLARIFY,
                confidence=0.3,
