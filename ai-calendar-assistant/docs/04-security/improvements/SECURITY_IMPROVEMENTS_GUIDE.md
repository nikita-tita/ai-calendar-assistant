# 🔒 Руководство по улучшениям безопасности

## Обзор

Этот документ описывает все реализованные улучшения безопасности для AI Calendar Assistant.

**Дата внедрения:** 28 октября 2025
**Версия:** 2.0 (Security Hardened)

---

## ✅ Реализованные улучшения

### 1. 🔐 Защита административных паролей

**Проблема:** Пароли хранились в открытом виде в исходном коде [admin.py](app/routers/admin.py)

**Решение:**
- Пароли перенесены в переменные окружения
- Хранятся в `.env` (исключен из git)
- Валидация при запуске приложения

**Файлы:**
- [app/routers/admin.py](app/routers/admin.py) - обновлен для чтения из env
- [.env](.env) - добавлены `ADMIN_PASSWORD_1`, `ADMIN_PASSWORD_2`, `ADMIN_PASSWORD_3`

**Использование:**
```bash
# В .env:
ADMIN_PASSWORD_1=ваш_основной_пароль
ADMIN_PASSWORD_2=ваш_вторичный_пароль
ADMIN_PASSWORD_3=ваш_третичный_пароль
```

---

### 2. 🌐 Закрыт публичный доступ к Radicale

**Проблема:** Radicale CalDAV сервер был доступен публично на порту 5232

**Решение:**
- Radicale доступен только внутри Docker сети
- Внешний порт закрыт
- Защита от брутфорса и DDoS

**Файлы:**
- [docker-compose.yml](docker-compose.yml) - обновлена конфигурация сети

**До:**
```yaml
radicale:
  ports:
    - "5232:5232"  # ❌ Публичный доступ
```

**После:**
```yaml
radicale:
  expose:
    - "5232"  # ✅ Только internal
  networks:
    - internal
```

---

### 3. 💾 Автоматическое резервное копирование

**Проблема:** Отсутствовала система backup - риск полной потери данных

**Решение:**
- Ежедневные автоматические бэкапы (3:00 AM)
- Шифрование с GPG
- Хранение 30 дней
- Возможность загрузки в облако

**Файлы:**
- [backup-calendar.sh](backup-calendar.sh) - скрипт создания бэкапа
- [restore-from-backup.sh](restore-from-backup.sh) - скрипт восстановления

**Что резервируется:**
- Radicale data (все календарные события)
- Bot data (настройки пользователей, аналитика)
- Конфигурация (.env, docker-compose.yml)

**Использование:**

```bash
# Ручной запуск бэкапа
./backup-calendar.sh

# Просмотр бэкапов
ls -lh /root/backups/calendar-assistant/

# Восстановление
./restore-from-backup.sh /root/backups/calendar-assistant/20251028_030000.tar.gz.gpg
```

**Cron job:**
```bash
# Добавлен автоматически при деплое
0 3 * * * /root/ai-calendar-assistant/backup-calendar.sh
```

---

### 4. 🔒 Шифрование данных

**Проблема:** JSON файлы хранились в открытом виде

**Решение:**
- Новый сервис `EncryptedStorage`
- Fernet (symmetric encryption)
- Автоматическая миграция существующих файлов
- Ротация ключей шифрования

**Файлы:**
- [app/services/encrypted_storage.py](app/services/encrypted_storage.py)

**Использование:**

```python
from app.services.encrypted_storage import encrypted_storage

# Сохранить зашифрованные данные
data = {"user_id": "12345", "preferences": {...}}
encrypted_storage.save(data, "user_prefs.json", encrypt=True)

# Загрузить (автоматическая расшифровка)
data = encrypted_storage.load("user_prefs.json")

# Ротация ключа (при компрометации)
encrypted_storage.rotate_key()
```

**Миграция существующих файлов:**
```python
# Автоматическая миграция при первой загрузке
# Незашифрованный файл загружается и пересохраняется зашифрованным
data = encrypted_storage.load("old_file.json")
# -> old_file.json.enc создан, old_file.json удален
```

---

### 5. 📝 Ротация логов

**Проблема:** Логи накапливались без ограничений

**Решение:**
- Ежедневная ротация
- Сжатие старых логов
- Хранение 30 дней для app logs, 14 для nginx
- Автоматическая очистка

**Файлы:**
- [logrotate-calendar.conf](logrotate-calendar.conf)

**Конфигурация:**
```bash
# Установка
sudo cp logrotate-calendar.conf /etc/logrotate.d/calendar-assistant

# Тестирование
sudo logrotate -d /etc/logrotate.d/calendar-assistant

# Принудительная ротация
sudo logrotate -f /etc/logrotate.d/calendar-assistant
```

---

### 6. 🔑 Уникальный SECRET_KEY

**Проблема:** Использовался дефолтный SECRET_KEY из кода

**Решение:**
- Сгенерирован криптографически стойкий ключ
- 32 байта (256 бит)
- Хранится в .env

**До:**
```python
SECRET_KEY=default-secret-key-change-in-production
```

**После:**
```python
SECRET_KEY=EtSGpUpspFDzH_Qf71FS47smg31X9oZ7VW7kkeR83AU
```

---

## 🚀 Развертывание

### Автоматическое развертывание

Используйте скрипт для автоматического деплоя всех улучшений:

```bash
./deploy-security-improvements.sh
```

Скрипт выполнит:
1. ✅ Backup текущего состояния
2. ✅ Загрузку обновленных файлов
3. ✅ Настройку прав доступа
4. ✅ Установку logrotate
5. ✅ Настройку cron для бэкапов
6. ✅ Перезапуск контейнеров
7. ✅ Проверку работоспособности
8. ✅ Создание первого бэкапа

### Ручное развертывание

Если нужно развернуть вручную:

```bash
# 1. Backup текущего состояния
ssh root@server 'cd /root/ai-calendar-assistant && docker-compose down'
ssh root@server 'tar czf /root/backup-$(date +%Y%m%d).tar.gz /root/ai-calendar-assistant'

# 2. Загрузка файлов
scp app/routers/admin.py root@server:/root/ai-calendar-assistant/app/routers/
scp app/services/encrypted_storage.py root@server:/root/ai-calendar-assistant/app/services/
scp docker-compose.yml .env root@server:/root/ai-calendar-assistant/
scp backup-calendar.sh restore-from-backup.sh root@server:/root/ai-calendar-assistant/

# 3. Установка скриптов
ssh root@server 'cd /root/ai-calendar-assistant && chmod +x *.sh'

# 4. Установка logrotate
scp logrotate-calendar.conf root@server:/etc/logrotate.d/calendar-assistant

# 5. Настройка cron
ssh root@server '(crontab -l; echo "0 3 * * * /root/ai-calendar-assistant/backup-calendar.sh") | crontab -'

# 6. Перезапуск
ssh root@server 'cd /root/ai-calendar-assistant && docker-compose up -d --build'

# 7. Первый бэкап
ssh root@server '/root/ai-calendar-assistant/backup-calendar.sh'
```

---

## 🧪 Тестирование

### Проверка паролей из env

```bash
# Запустить контейнер
docker-compose up -d

# Проверить логи на ошибки паролей
docker-compose logs telegram-bot | grep -i "password"

# Должно быть: "encrypted_storage_initialized" и НЕ должно быть ошибок
```

### Проверка закрытия Radicale порта

```bash
# На сервере проверить открытые порты
netstat -tlnp | grep 5232

# Не должно быть строк с 0.0.0.0:5232

# Проверка извне (должна FAILИТЬ)
curl http://95.163.227.26:5232
# Connection refused - ✅ правильно

# Проверка изнутри Docker (должна работать)
docker exec telegram-bot curl http://radicale:5232
# Должен вернуть ответ - ✅ правильно
```

### Проверка бэкапа

```bash
# Запустить ручной бэкап
./backup-calendar.sh

# Проверить созданный файл
ls -lh /root/backups/calendar-assistant/

# Тест восстановления (ОПАСНО - делать только на staging!)
./restore-from-backup.sh /root/backups/calendar-assistant/latest.tar.gz.gpg
```

### Проверка шифрования

```bash
# Проверить что файлы зашифрованы
docker exec telegram-bot ls -la /var/lib/calendar-bot/

# Должны быть файлы с расширением .enc
# user_preferences.json.enc
# analytics_data.json.enc

# Попытка прочитать напрямую (должно быть нечитаемо)
docker exec telegram-bot cat /var/lib/calendar-bot/user_preferences.json.enc
# Бинарный мусор - ✅ зашифровано
```

---

## 📊 Мониторинг

### Проверка бэкапов

```bash
# Просмотр логов бэкапа
tail -f /var/log/calendar-backup.log

# Список бэкапов
ls -lh /root/backups/calendar-assistant/

# Проверка размера бэкапов
du -sh /root/backups/calendar-assistant/
```

### Проверка ротации логов

```bash
# Просмотр ротированных логов
ls -lh /var/log/calendar-assistant/

# Должны быть файлы:
# app.log
# app.log.1.gz
# app.log.2.gz
```

### Проверка cron jobs

```bash
# Просмотр установленных cron jobs
crontab -l

# Должна быть строка:
# 0 3 * * * /root/ai-calendar-assistant/backup-calendar.sh
```

---

## 🆘 Восстановление при инциденте

### 1. Компрометация паролей

```bash
# Сгенерировать новые пароли
python3 -c "import secrets; print(secrets.token_urlsafe(16))"

# Обновить .env
nano /root/ai-calendar-assistant/.env

# Обновить ADMIN_PASSWORD_1, ADMIN_PASSWORD_2, ADMIN_PASSWORD_3

# Перезапустить
docker-compose restart
```

### 2. Компрометация ключа шифрования

```python
# Войти в контейнер
docker exec -it telegram-bot python3

# Ротация ключа
from app.services.encrypted_storage import encrypted_storage
encrypted_storage.rotate_key()

# Backup старого ключа сохранится в .encryption_key.backup
```

### 3. Потеря данных

```bash
# Восстановить из последнего бэкапа
./restore-from-backup.sh /root/backups/calendar-assistant/$(ls -t /root/backups/calendar-assistant/ | head -1)

# Проверить восстановление
docker-compose ps
docker-compose logs -f
```

---

## 📋 Чеклист после развертывания

- [ ] Веб-приложение работает: https://этонесамыйдлинныйдомен.рф
- [ ] Админ-панель доступна с новыми паролями
- [ ] Бэкапы создаются автоматически (проверить через 24 часа)
- [ ] Radicale НЕ доступен извне на порту 5232
- [ ] Логи ротируются (проверить через неделю)
- [ ] Бот отвечает на команды в Telegram
- [ ] События создаются и синхронизируются

---

## 🔐 Рейтинг безопасности

### До улучшений: 6/10 ⚠️

- ❌ Пароли в коде
- ❌ Нет бэкапов
- ❌ Radicale публичный
- ❌ Данные не зашифрованы
- ✅ Трехфакторная аутентификация
- ✅ PII masking

### После улучшений: 9/10 ✅

- ✅ Пароли в environment variables
- ✅ Ежедневные зашифрованные бэкапы
- ✅ Radicale только internal
- ✅ Данные зашифрованы
- ✅ Трехфакторная аутентификация
- ✅ PII masking
- ✅ Log rotation
- ✅ Уникальный SECRET_KEY

---

## 🎯 Следующие шаги (опционально)

### Продвинутые улучшения

1. **Fail2ban** - защита от брутфорса
```bash
apt install fail2ban
# Настроить для Nginx и Docker
```

2. **Мониторинг** - Prometheus + Grafana
```bash
# Добавить метрики
# Алерты на email/Telegram
```

3. **2FA для пользователей** - TOTP через Telegram
```python
# Добавить pyotp
# QR коды для регистрации
```

4. **Rate limiting с Redis**
```python
# Уже есть в коде
# Настроить лимиты
```

5. **WAF** - ModSecurity для Nginx
```bash
# Защита от OWASP Top 10
```

---

## 📚 Дополнительные ресурсы

- [OWASP Top 10](https://owasp.org/Top10/)
- [Docker Security Best Practices](https://docs.docker.com/engine/security/)
- [Cryptography Library Docs](https://cryptography.io/)
- [Radicale Documentation](https://radicale.org/v3.html)

---

## 📞 Поддержка

При возникновении проблем:

1. Проверить логи: `docker-compose logs -f`
2. Проверить backup logs: `tail -f /var/log/calendar-backup.log`
3. Проверить cron: `crontab -l`
4. Восстановить из бэкапа если нужно

**Дата обновления:** 28 октября 2025
**Версия:** 2.0
